# %%
# Interactive usage examples for QuantumSl2Quasi (U_q^{(Phi)}sl(2)) --
...
# this one algebra. See quantum_sl2_quasi.py's module docstring for the
# full definition.
...
def _find_src_dir() -> Path:
    ...
    plain script, `python3 quantum_sl2_quasi_usage.py`); the current
...
print("E^p                      =", E ** alg.p)
print("F^p                      =", F ** alg.p)
print("K^(2p) - 1               =", K ** (2 * alg.p) - alg.unit())

# %%
# Example coproducts. Delta(K) = K (x) K (grouplike, same as an honest
# Hopf algebra); Delta(E)/Delta(F) pick up the central idempotents
# e0 = (1+K^p)/2, e1 = (1-K^p)/2 and the gauge parameter t -- this
# t-dependence (and Phi's non-triviality below) is exactly what makes
# this algebra "quasi" rather than an honest Hopf algebra. Non-zero, so
...
# %%
# The associator Phi -- trivial (1 (x) 1 (x) 1) would mean an honest
# Hopf algebra; here it genuinely isn't.
...
# ribbon element) -- not part of check_all since not every algebra has
# it, but QuantumSl2Quasi does. Unlike check_all, these only print
# internally on *failure*, so wrap in print() to see the (expected) True
# on success too.
